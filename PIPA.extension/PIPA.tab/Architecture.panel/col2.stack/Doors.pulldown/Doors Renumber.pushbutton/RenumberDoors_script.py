...
def RenumberElements(elm_list):
    ...
    start = int(forms.ask_for_string(default="1", prompt='Enter starting number (integers only):', title='Starting sequence'))
    
    floatFill = float(fill)
    
    for i, e in enumerate(elm_list):
        e.get_Parameter(BuiltInParameter.ALL_MODEL_MARK).Set(prefix+str(start + i).zfill(floatFill))
