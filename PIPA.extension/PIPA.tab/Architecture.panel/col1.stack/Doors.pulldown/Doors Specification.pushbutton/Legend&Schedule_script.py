...
output = DB.FilteredElementCollector(doc).OfCategory(DB.BuiltInCategory.OST_Doors).WhereElementIsNotElementType().ToElements()
...
for ut,ft in unique_types(output):
    ...
    table_data=doc.GetElement(new_schedule).GetTableData() \
											...
											.SetCellText(0,0,'Спецификация врати - Tип '+ut)
