...
for door in allDoors:
    if door.Symbol not in allDoorTypes:
        allDoorTypes.append(door.Symbol)
...
if len(finalDoorsList) != 1:
    reportMsg = str(len(finalDoorsList) - 1) + " new Legends and schedules were created successfully! Enjoy ;)"
elif len(finalDoorsList) == 1:
...
if nonIpaDoorNamesList != []:
    reportMsg += '\n\nThe following door families were found in the project, not being subject to IPA copyrights. Please contact the BIM team for evaluation of these families: ' + str(nonIpaDoorNamesList)
