...
newPlanOption = forms.CommandSwitchWindow.show(
    ["Create new Floor / Area plan", "Duplicate the Active View with Detailing", "Duplicate as a Dependent"],
...
if newPlanOption == "Create new Floor / Area plan":
    boolNewPlan = True
elif newPlanOption == "Duplicate the Active View with Detailing":
    boolDetailing = True
elif newPlanOption == "Duplicate as a Dependent":
    boolDependent = True
...
for a in elements:
    ...
    if boolNewPlan:
        ...
        AreaPlan = doc.GetElement(doc.ActiveView.Duplicate(ViewDuplicateOption.AsDependent))
        AreaPlan.CropBoxActive = False
        AreaPlan.ViewTemplateId = ElementId(-1)
        AreaPlan.get_Parameter(BuiltInParameter.VIEWER_VOLUME_OF_INTEREST_CROP).Set(ElementId(-1))
        AreaPlan.get_Parameter(BuiltInParameter.PLAN_VIEW_NORTH).Set(0)
