	...
	if room.Level.Id == doc.ActiveView.GenLevel.Id:
		for area in areas:
			if IsRoomInArea(area,room) == True:
				room.get_Parameter(DB.BuiltInParameter.ROOM_DEPARTMENT).Set(area.LookupParameter("Name").AsString())
				if room.get_Parameter(DB.BuiltInParameter.ROOM_DEPARTMENT).AsString() not in room.get_Parameter(DB.BuiltInParameter.ROOM_NAME).AsString():
					room.get_Parameter(DB.BuiltInParameter.ROOM_NAME).Set(area.LookupParameter("Name").AsString()+"-"+room.get_Parameter(DB.BuiltInParameter.ROOM_NAME).AsString())
				else:
					pass
